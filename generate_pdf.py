...
def cover_page(styles, metadata):
    ...
    stats = [
        [
            _stat_box(str(total), "Papers reviewed", LIGHT_GREY, NAVY),
            _stat_box(str(hep_count), "Hepatology papers", LIGHT_TEAL, TEAL),
            _stat_box(str(gi_count), "Luminal GI papers", LIGHT_GOLD, GOLD),
        ]
    ]
...
def generate_pdf(digest_data: dict, output_path: str) -> str:
    ...
    metadata["gi_count"] = len(luminal_gi)
    ...
    story.append(section_header_block(
        ...
        f"{len(luminal_gi)} papers",
        TEAL
    ))
    story.append(Spacer(1, 6 * mm))

    if luminal_gi:
        story.extend(highlights_box(luminal_gi, styles, TEAL, LIGHT_GOLD))
        story.append(Spacer(1, 8 * mm))
        render_papers(luminal_gi, story, styles)
    else:
        story.append(Paragraph("No luminal GI papers found this week.", styles["body"]))

    # ── Guidelines Section ─────────────────────────────────────────────────────
    if guidelines:
        story.extend(guidelines_section(guidelines, styles))
    else:
        # Still show the section with a "none this week" message
        story.append(PageBreak())
        story.extend(guidelines_section([], styles))
